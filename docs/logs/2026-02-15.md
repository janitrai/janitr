...
Opened new PR for post-merge follow-up build-formatting change.

- Branch: `fix/extension-build-autoformat-js`
- PR: https://github.com/janitrai/janitr/pull/26
- Title: `chore(extension): auto-format generated JS during extension build`

Auth notes:

- Used existing GitHub App credential helper configured in git (`~/.config/github-app/git-credential-github-app`) to authenticate GitHub CLI non-interactively.
- Active GH account for CLI in this environment: `janitrbob[bot]`.

## 23:00

Adjusted README install instructions formatting per user request.

Change made:

- Replaced inline shell command items in `README.md` Install section with a fenced `bash` code block:
  - `corepack enable`
  - `pnpm install`
  - `pnpm extension:build`
- Kept browser/manual steps as numbered text below the shell block.

Validation:

- `pnpm run format:check` passed.

Opened new PR for README install command formatting update.

- Branch: `docs/readme-install-codeblock`
- PR: https://github.com/janitrai/janitr/pull/27
