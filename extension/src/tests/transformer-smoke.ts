...
const SCAM_SAMPLE =
  "URGENT AIRDROP: Connect wallet now and enter your seed phrase to claim rewards. " +
  "Limited time, DM for instant whitelist access.";
...
const sendMessage = (message: Record<string, unknown>) =>
  new Promise<RuntimeMessageResponse>((resolve, reject) => {
    ...
    chrome.runtime.sendMessage(message, (response: RuntimeMessageResponse) => {
...
const pickRunId = (runs: RuntimeMessageResponse["runs"]): string | null => {
  const candidates = (Array.isArray(runs) ? runs : [])
    .filter((run) => run && run.is_transformer_candidate !== false)
    .map((run) => ({
      runId: String(run?.run_id || "").trim(),
      totalBytes: Number(run?.total_bytes || Number.POSITIVE_INFINITY),
    }))
...
const run = async (): Promise<void> => {
  try {
    ...
    if (result.label !== "scam") {
      fail(
        `Expected label "scam" for obvious scam sample, got "${String(result.label)}" ` +
          `(score=${String(scamScore)}, threshold=${String(scamThreshold)}).`,
      );
    }
