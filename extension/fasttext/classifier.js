...
const buildThresholds = (perLabel, globalThreshold) => {
  ...
  for (const label of CLASSES) {
    ...
    const value = perLabel ? perLabel[label] : void 0;
    const numeric = Number(value);
    thresholds[label] = Number.isFinite(numeric) ? numeric : 1;
  }
  return thresholds;
};
const loadClassifierThresholds = async ({ thresholdsUrl, modelUrl } = {}) => {
  if (!configPromise) {
    configPromise = loadConfig({ thresholdsUrl, modelUrl }).then((config2) => {
      ...
      cachedThresholds = config2.thresholds;
      return config2;
    });
  }
...
const getClassifierThresholds = () => cachedThresholds;
const getClassifierConfig = () => cachedConfig;
const readPredictionScores = (model, text, k) => {
  const rawPredictions = model.predict(text, k, 0);
  const scores = {};
