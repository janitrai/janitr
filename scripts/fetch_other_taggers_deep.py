...
def main():
    ...
    with open(output_file, "a") as out:
        for i, callout in enumerate(remaining):
            ...
            print(
                f"[{i + 1}/{len(remaining)}] @{tagger} → @{handle} (tweet {tweet_id})...",
                end=" ",
            )
            ...
            if not tweet_data:
                ...
                grandparent_extracted = (
                    extract_tweet_data(grandparent) if grandparent else None
                )

                if parent_extracted:
                    print(
                        f"@{parent_extracted['handle']}: {parent_extracted['text'][:60]}..."
                    )
                else:
                    print("No parent (deleted)")
    ...
    print(
        f"  With AI reply (parent): {with_parent}/{total} ({100 * with_parent // total}%)"
    )
