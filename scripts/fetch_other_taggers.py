...
def make_reply_entry(callout_data, ai_reply_data, original_data, flagged_handle, tagger):
    ...
    reply_id = ai_reply_data["status_id"] if ai_reply_data else callout_data["status_id"]
    return {
...
def main():
    ...
    for i, c in enumerate(callouts):
        ...
        if parent:
            ...
            print(f"  AI reply: @{ai_reply_extracted['handle']}: {ai_reply_extracted['text'][:80]}...")
            
            # Get grandparent (original post)
            grandparent = parent.get("parent")
            if grandparent:
                original_extracted = extract(grandparent)
                print(f"  Original: @{original_extracted['handle']}: {original_extracted['text'][:60]}...")
            else:
                ...
                if ai_reply_full:
                    ...
                    if gp:
                        ...
                        print(f"  Original (2nd fetch): @{original_extracted['handle']}: {original_extracted['text'][:60]}...")
        else:
        ...
        if ai_reply_extracted:
            entry = make_reply_entry(callout_extracted, ai_reply_extracted, original_extracted, handle, tagger)
