...
def batch_fetch_and_append(callouts_file: str):
    ...
    new_callouts = [
        c for c in callouts if c["flagged_handle"].lower() not in existing_handles
    ]
    print(
        f"Total callouts: {len(callouts)}, already have: {len(callouts) - len(new_callouts)}, to fetch: {len(new_callouts)}"
    )
