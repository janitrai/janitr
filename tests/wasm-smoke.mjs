...
const run = async () => {
  ...
  try {
    ...
    await page.waitForFunction('window.__wasmTestDone === true', { timeout: 120000 });
    ...
    assert(Array.isArray(results) && results.length > 0, 'No results returned.');
    ...
    for (const result of results) {
      ...
      if (result.probability >= result.threshold) {
        assert(result.isScam === true, 'isScam should be true when probability >= threshold.');
      }
    }
